...
const Multas = ({ onNavigate }) => {
  ...
  // Dados de multas por estado (valores aproximados em reais)
  ...
  // Gerar multas de exemplo
  const multas = useMemo(() => {
    const estados = Object.keys(valoresMultasPorEstado);
    const cidades = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Curitiba'];
    const tipos = Object.keys(tiposMulta);
    
    return Array.from({ length: 25 }, (_, i) => {
      const estado = estados[Math.floor(Math.random() * estados.length)];
      const cidade = cidades[Math.floor(Math.random() * cidades.length)];
      const tipo = tipos[Math.floor(Math.random() * tipos.length)];
      const valorConfig = valoresMultasPorEstado[estado]?.[cidade]?.[tipo] || { medio: 200 };
      
      const data = new Date();
      data.setDate(data.getDate() - Math.floor(Math.random() * 30));
      
      const statusOptions = ['pendente', 'paga', 'vencida', 'contestada'];
      const status = statusOptions[Math.floor(Math.random() * statusOptions.length)];
      
      return {
        id: `MUL-${String(i + 1000).padStart(4, '0')}`,
        placa: `ABC${Math.floor(Math.random() * 9000) + 1000}`,
        estado,
        cidade,
        tipo,
        descricao: tiposMulta[tipo].label,
        valor: valorConfig.medio + (Math.random() * 100 - 50),
        pontos: tiposMulta[tipo].pontos,
        data: data.toISOString().split('T')[0],
        status,
        local: `${cidade} - ${estado}`,
        veiculo: `Veículo ${Math.floor(Math.random() * 5) + 1}`,
        vencimento: new Date(data.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        desconto: status === 'pendente' && Math.random() > 0.5
      };
    });
  }, []);
  ...
  // Filtragem e ordenação - MOVER PARA CIMA
  ...
  // Calcular estatísticas - AGORA DEPOIS DE filteredAndSortedMultas
  const stats = useMemo(() => {
  ...
  // Função para calcular multa específica
                      ...
                      <td>
                        <div className="multas-tipo-info">
                          <div 
                            ...
                            style={{ backgroundColor: tiposMulta[multa.tipo].cor }}
                          />
