...
/**
 * Gera um relatório resumido de pagamentos
 * @param {Object} params - Parâmetros para geração do relatório
 * @param {string} params.periodo - Período do relatório ('hoje', 'semana', 'mes', 'ano', 'todos')
 * @param {string} params.status - Status dos pagamentos ('pendente', 'pago', 'cancelado', 'todos')
 * @returns {Promise<Blob>} Blob contendo o PDF do relatório
 */
export const gerarRelatorioPagamentos = async ({ periodo, status }) => {
    try {
        const response = await api.post('/multas/relatorios/pagamentos', {
            periodo,
            status,
            formato: 'pdf'
        }, {
